...
async function edonInteractive(opts) {
  const repl = require('repl');

  const {
    browser,
    page
  } = await (0, _util.getDefaultBrowserAndPage)(opts);
...
async function edonNoninteractive(opts) {
  const [stdin, {
    browser,
    page
  }] = await Promise.all([(0, _util.readStdin)(), (0, _util.getDefaultBrowserAndPage)(opts)]);
